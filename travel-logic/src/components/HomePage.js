...
class HomePage extends Component {
	...
	getPlaces = (e) => {
		e.preventDefault()
		fetch(this.state.baseUrl + `location_id=${this.state.cityName}&` + `start_date=${this.state.startDate}&` + `end_date=${this.state.endDate}`, {
			headers: {
				"X-Triposo-Account": process.env.REACT_APP_X_TRIPOSO_ACCOUNT,
				"X-Triposo-Token": process.env.REACT_APP_X_TRIPOSO_TOKEN
			}
			...
			<>
				...
				<div>
					<form onSubmit={this.getPlaces} className="search-form">
						<Autocomplete className="going-to-input" name="going-to" placeholder="📍 Going to" 
							...
							onPlaceSelected={(place) => {
								...
								this.state.cityName = place.address_components[0].long_name.replace(' ','_')
						...
						<input className="date-input" name="startDate" type="date" onChange={(event) => this.startDateChange(event)} />
						<label className="end-date-label">End Date</label>
						<input className="date-input" name="endDate" type="date" onChange={(event) => this.endDateChange(event)} />
						<div onClick={() => window.location.replace("/#map-container")}>
						<input className="submit-input" name="search-button" type="submit" value="Search" />
						</div>
					</form>
				</div>
